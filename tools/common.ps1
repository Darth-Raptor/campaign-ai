...
function Find-CampaignAIFileBank {
    $defaultFileBank = "${env:ProgramFiles(x86)}\Steam\steamapps\common\Arma 3 Tools\FileBank\FileBank.exe"
    if (Test-Path -LiteralPath $defaultFileBank) {
        return $defaultFileBank
    }

    $cmd = Get-Command "FileBank.exe" -ErrorAction SilentlyContinue
    if ($cmd) {
        return $cmd.Source
    }
